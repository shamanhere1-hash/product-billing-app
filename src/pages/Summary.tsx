...
export default function Summary() {
  ...
  const getTodaysBills = () => {
    ...
    return billedOrders.filter(bill => format(new Date(bill.createdAt), 'yyyy-MM-dd') === today);
  };
  ...
  const getDayWiseTotals = () => {
    ...
    return DAYS_OF_WEEK.map(day => {
      const dayBills = weeklyBills.filter(bill => format(new Date(bill.createdAt), 'EEEE') === day);
      return {
        day,
  ...
  const weeklyBills = getWeeklyBills();
  const todayTotal = todaysBills.reduce((sum, bill) => sum + Number(bill.total), 0);
  const weeklyTotal = weeklyBills.reduce((sum, bill) => sum + Number(bill.total), 0);
          ...
          <Tabs defaultValue="daily" className="w-full">
              ...
              <div className="bg-card rounded-xl border border-border p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    <p className="text-3xl font-bold text-primary">₹{todayTotal.toFixed(2)}</p>
            ...
            <TabsContent value="weekly" className="space-y-6">
              <div className="bg-card rounded-xl border border-border p-6">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {format(startOfWeek(new Date(), { weekStartsOn: 1 }), 'dd MMM')} - {format(endOfWeek(new Date(), { weekStartsOn: 1 }), 'dd MMM yyyy')}
                </p>

                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                    <p className="text-3xl font-bold text-primary">₹{weeklyTotal.toFixed(2)}</p>
                    <p className="text-sm text-muted-foreground mt-1">Total Sales</p>
                  </div>
                  <div className="bg-muted/50 rounded-lg p-4 text-center">
                ...
                <div className="space-y-2">
                  {dayWiseTotals.map(({ day, bills, total }) => (
                    <div key={day} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                      ...
                      <div className="text-right">
                        <span className="text-muted-foreground mr-4">{bills} bills</span>
                ...
                {weeklyProducts.length > 0 && (
                  <div className="mt-6">
                    <h4 className="font-medium mb-3">Top Selling Products (Week)</h4>
                    <div className="space-y-2">
