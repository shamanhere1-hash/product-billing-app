...
import { ChevronRight, Calendar, FileText, User, Clock, Loader2, ChevronDown } from 'lucide-react';
...
export default function History() {
  ...
  const getGroupedOrders = () => {
    const billedOrders = orders
      .filter(o => o.status === 'billed' || o.status === 'packed' || o.status === 'pending') // For debugging, usually just 'billed'. User requested completed orders.
      .filter(o => o.status === 'billed') // STRICTLY BILLED as per request
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

    const groups: { month: string; dates: { date: string; displayDate: string; orders: Order[] }[] }[] = [];
    ...
    <div className="page-container">
              ...
              <div key={monthGroup.month} className="space-y-4">
                ...
                <div className="space-y-3">
                  {monthGroup.dates.map(dateGroup => {
                    ...
                    const totalRevenue = dateGroup.orders.reduce((sum, o) => sum + o.total, 0);

                    return (
                      <div key={dateGroup.date} className="bg-card rounded-xl border border-border overflow-hidden">
                        <button
                          onClick={() => setSelectedDate(isExpanded ? null : dateGroup.date)}
                          className="w-full flex items-center justify-between p-4 hover:bg-muted/50 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <Calendar className="w-5 h-5 text-primary" />
                            <span className="font-medium text-lg">{dateGroup.displayDate}</span>
                          </div>
                          <div className="flex items-center gap-4">
                            <span className="text-sm text-muted-foreground">{dateGroup.orders.length} bills</span>
                              ...
                              >
                                <div className="flex items-center gap-4">
                                  ...
                                  <div className="text-left">
                                    <p className="font-mono font-medium text-foreground">{bill.orderNumber}</p>
      ...
      <Dialog open={!!selectedBill} onOpenChange={() => setSelectedBill(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="font-mono">{selectedBill?.orderNumber}</DialogTitle>
          </DialogHeader>
          ...
          {selectedBill && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span>{format(new Date(selectedBill.createdAt), 'dd MMM yyyy')}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Time</span>
                <span>{format(new Date(selectedBill.createdAt), 'hh:mm a')}</span>
              </div>
                  ...
                  {selectedBill.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-sm py-1 border-b border-border/30 last:border-0">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">Qty: {item.quantity} × ₹{item.overriddenPrice ?? item.product.price}</div>
                      </div>
                      <span className="font-medium">₹{((item.overriddenPrice ?? item.product.price) * item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
